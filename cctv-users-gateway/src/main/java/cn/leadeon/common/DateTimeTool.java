/*
 * 文 件 名:  DateTimeTool.java
 * 版    权:  Xi'An Leadeon Technologies Co., Ltd. Copyright 2014-12-29,  All rights reserved  
 */
...
public class DateTimeTool {
     ...
     * 1 天 30 * 24 * 3600 秒
     */
	public static long MONTH_SEC_NUMBER = DAY_SEC_NUMBER * 30;

    /**
     * 1 年 365 * 24 * 3600 秒
     */
	public static long YEAR_SEC_NUMBER = DAY_SEC_NUMBER * 365;
    ...
    public static String getCurrentDateStr(String dateStr) {
        //得到日期
     ...
     * @return 日期
     */
    public static java.util.Date toDateTimeByPattern(String strDate, String pattern) {
        try {
            //得到日期
            SimpleDateFormat formatter = new SimpleDateFormat(pattern);
    ...
    public static int getCurrDateInt(){
        Long date=new java.util.Date().getTime()/1000;
    ...
    public static String getDate (String oTime) {
        ...
        StringBuffer buff = new StringBuffer(oTime);
        String sfstr = "";
        try {
         if (buff.length() == 12) {
        	 buff.append("00");
         }
         sfstr = sf2.format(sf1.parse(buff.toString()));
    ...
    public static String getDate (String oTime, int flag) {
    ...
    public static String getDate (String oTime, String flag) {
		SimpleDateFormat sf1 = new SimpleDateFormat("yyyyMMddHHmmss");
        SimpleDateFormat sf2 =new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        StringBuffer buff = new StringBuffer(oTime);
        String sfstr = "";
    ...
    public static String findYearMonth()
    {
         ...
         * 拼接年份和月份
         */
        date = String.valueOf(year)  + ( month<10 ? "0" + month : month);
        /**
         * 返回当前年月
         */
        return date;
...
//    	sb.insert(13, ':');
//    	sb.insert(36, ':');
    	//System.out.println(DateTimeTool.getDate("20150501",0));
//    	List<String> dd = getLeftMon("201410","201502");
//    	//System.out.println(getAllDate("02"));
...
//    	System.out.println(dd);
    	System.out.println(getDates("2015122212"));
    }
     ...
     * @see [类、类#方法、类#成员]
     */
    public static List<String> getLeftMon(String startMon,String endMon) {
    	...
    	try {
			...
			java.util.Date starDate = sdf.parse(startMon); 
			java.util.Date endDate = sdf.parse(endMon);  
			...
			while(endCalendar.compareTo(starCalendar)>=0){
				String date = sdf.format(starCalendar.getTime());
    ...
    public static String getLeftYear(String startMon,String endMon, String key) {
    	...
    	try {
			...
			for (Map.Entry<String, String> maps : map.entrySet()) {
				if (StringUtils.equals(key, maps.getKey())) {
					year = maps.getValue();
				}
			}
		} catch (Exception e) {
     ...
     * @see [类、类#方法、类#成员]
     */
    public static String getAllDate(String mon) {
    	Map<String , String> map = new HashMap<>();
    	String result = "";
    	map.put("01", "01-01@!@01-31");
    	map.put("02", "02-01@!@02-28");
    	map.put("03", "03-01@!@03-31");
    	map.put("04", "04-01@!@04-30");
    	map.put("05", "05-01@!@05-31");
    	map.put("06", "06-01@!@06-30");
    	map.put("07", "07-01@!@07-31");
    	map.put("08", "08-01@!@08-31");
    	map.put("09", "09-01@!@09-30");
    	map.put("10", "10-01@!@10-31");
    	map.put("11", "11-01@!@11-30");
    	map.put("12", "12-01@!@12-31");
    	
    	for(Map.Entry<String, String> entry:map.entrySet()){ 
    		if (StringUtils.equals(mon, entry.getKey())) {
     ...
     * @return
     */
    public static String getDates (String oTime) {
		SimpleDateFormat sf1 = new SimpleDateFormat("yyyy-MM-dd");
        StringBuffer buff = new StringBuffer();
        String sfstr = "";
        int length = 0;
        try {
         if (oTime != null && (length = oTime.length()) > 0 && length <= 8) {
        	 String year = oTime.substring(0, (length-4) >= 0 ? 4:length);
     		 String month = oTime.substring(length >= 4 ? 4 : length, (length-6) >= 0 ? 6:length);
     		 String day = oTime.substring(length >= 6 ? 6 : length, length);
     		 buff.append(year + "-" + month + "-" + day);
        	 sfstr = sf1.format(sf1.parse(buff.toString()));
         }else if(length > 8 && length <= 14){
        	 String year = oTime.substring(0,(length-4) >= 0 ? 4:length);
     		 String month = oTime.substring(length >= 4 ? 4:length, (length-6) >= 0 ? 6:length);
     		 String day = oTime.substring(length >= 6 ? 6 : length, (length-8) >= 0 ? 8:length);
     		 String hourse = oTime.substring(length >= 8 ? 8 : length,(length-10) >= 0 ? 10:length);
     		 String miniter = oTime.substring(length >= 10 ? 10 : length,(length-12) >= 0 ? 12:length);
     		 String seconds = oTime.substring(length >= 12 ? 12 : length,length);
     		DateTime data = new DateTime(Integer.parseInt(!StringUtils.isEmpty(year) ? year :"0"),
   				 Integer.parseInt(!StringUtils.isEmpty(month) ? month : "0"),
   				 Integer.parseInt(!StringUtils.isEmpty(day) ? day : "0"), 
   				 Integer.parseInt(!StringUtils.isEmpty(hourse) ? hourse : "0"), 
   				 Integer.parseInt(!StringUtils.isEmpty(miniter) ? miniter : "0"), 
   				 Integer.parseInt(!StringUtils.isEmpty(seconds) ? seconds : "0"));
        	 sfstr = data.toString("yyyy-MM-dd HH:mm:ss");
         }
	     } catch (Exception e) {
	      // TODO Auto-generated catch block
	      e.printStackTrace();
